...
export class FlowCesium {
  ...
  prepareResource(gl) {
    ...
    const projectionTexture = createMyTexture(gl, 1, gl.TEXTURE_2D, WebGL2RenderingContext.RGB32F, 0, 0);
    fillTexture(
      gl,
      this._imageMap.get(FlowEnum.PROJECTION_CESIUM_IMAGE),
      "Float_Point",
      0,
      this.option.textureSize.projection[0],
      this.option.textureSize.projection[1],
    ...
    this._textureMap.set(FlowEnum.PROJECTION_CESIUM_TEXTURE, projectionTexture);
  ...
  tickRender(gl, matrix) {
    ...
    gl.bindSampler(0, this._samplerMap.get(FlowEnum.N_SAMPLER));
    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_2D, this._textureMap.get(FlowEnum.PROJECTION_CESIUM_TEXTURE));
    gl.bindSampler(1, this._samplerMap.get(FlowEnum.L_SAMPLER));
    gl.disable(gl.DEPTH_TEST);
    gl.enable(gl.BLEND);
    gl.blendColor(0.0, 0.0, 0.0, 0.0);
...
export class FlowOpenLayers extends WebGLTileLayer {
  ...
  prepareResource() {
    if (this._simulationVAO && this._simulationVAO2 && this._renderVAO && this._renderVAO2) return;
    const gl = this._gl;
    const available_extensions = gl.getSupportedExtensions();
    for (const extension of available_extensions) {
      gl.getExtension(extension);
    ...
    fillTexture(
      gl,
      this._imageMap.get(FlowEnum.PROJECTION_OL_IMAGE),
      "Float_Point",
      0,
      this.option.textureSize.projection[0],
      this.option.textureSize.projection[1],
    ...
    this._textureMap.set(FlowEnum.PROJECTION_OL_TEXTURE, projectionTexture);
  ...
  tickRender(gl, frameState) {
    ...
    gl.bindSampler(0, this._samplerMap.get(FlowEnum.N_SAMPLER));
    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_2D, this._textureMap.get(FlowEnum.PROJECTION_OL_TEXTURE));
    gl.bindSampler(1, this._samplerMap.get(FlowEnum.L_SAMPLER));
    gl.disable(gl.DEPTH_TEST);
    gl.enable(gl.BLEND);
    gl.blendColor(0.0, 0.0, 0.0, 0.0);
  ...
  render(frameState, target) {
    ...
    if (!this._gl) this._gl = target.getContext("webgl2");
    else {
      this.prepareResource();
      this.tickLogicCount(this._gl);
      this.tickRender(this._gl, frameState);
    }
    frameState.postRenderFunctions.push((map, frameState) => {
      map.render();
    });
    return canvas;
  }
