...
//        }
        ...
        void display(T* dd)
        {
            ...
            bounding_rect(&minx,&miny,&maxx, &maxy);
            m_curved_count.count(0);
            mtx=dd->begin(minx,miny,maxx,maxy);
            for(int i = 0; i < m_attr_storage.size(); i++)
            {
                ...
                {   
                    ...
                    {
                        shaping::path2d_agg_adaptor_t<curved_trans> path(m_curved_trans);
                        m_curved_trans.rewind(attr.index);
                        dd->display(color,&path,attr.even_odd_flag ? fill_even_odd : fill_non_zero);
                    }
                    else
                    {
                        m_curved_trans_contour.miter_limit(attr.miter_limit);
                        m_curved_trans_contour.rewind(attr.index);
                        shaping::path2d_agg_adaptor_t path(m_curved_trans_contour);
                        dd->display(color,&path,attr.even_odd_flag ? fill_even_odd : fill_non_zero);
                    }
                }
                ...
                if(attr.stroke_flag)
                {
                    ...
                    m_curved_stroked_trans.rewind(attr.index);
                    shaping::path2d_agg_adaptor_t path(m_curved_stroked_trans);
                    dd->display(color,&path,fill_non_zero);
                }
            }
            dd->end();
        }
