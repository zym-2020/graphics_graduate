...
namespace agg
{
    namespace svg
    {
        ...
        void parser::end_element(void *data, const char *el)
        {
            ...
            {
                self.m_css_flag = false;
                printf("%s", self.m_css_cache.c_str());
            }
            if (strcmp(el, "g") == 0)
            {
        ...
        void parser::parse_path(const char *p)
        {
            m_tokenizer.set_path_str(p);
        ...
        bool is_param_string(const char *str)
        {
            return str[0] == 'p' &&
                   str[1] == 'a' &&
                   str[2] == 'r' &&
        ...
        void parser::parse_rect(const char **attr)
        {
            ...
            {
                if (w < 0.0)
                    throw exception("parse_rect: Invalid width: %f", w);
                if (h < 0.0)
                    throw exception("parse_rect: Invalid height: %f", h);
            ...
            for (i = 0; attr[i]; i += 2)
            {
                if (!parse_attr(attr[i], attr[i + 1]))
                {
                    if (strcmp(attr[i], "cx") == 0)
                        cx = parse_double(attr[i + 1]);
                    if (strcmp(attr[i], "cy") == 0)
                        cy = parse_double(attr[i + 1]);
                    if (strcmp(attr[i], "r") == 0)
                        r = parse_double(attr[i + 1]);
                }
            }
